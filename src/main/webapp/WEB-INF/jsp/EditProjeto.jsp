...
<div class="container">
    ...
    <h1 class="p-3"> Edit Projeto </h1>

    <form:form action="/projeto/editSave" method="post" onsubmit="return validarProjeto()" modelAttribute="projeto">
                ...
                <div class="col-md-6">
                    <select path="status" id="status"
                        ...
                        <c:forEach items="${statusList}" var="status">
                            <option value="${status}" ${projeto.status == status ? 'selected' :''}>${status}</option>
                        </c:forEach>
                    </select>
                </div>
            </div>
                ...
                <div class="col-md-6">
                    <select path="risco" id="risco"
                        ...
                        <c:forEach items="${riscoList}" var="risco">
                            <option value="${risco}" ${projeto.risco == risco ? 'selected' :''}>${risco}</option>
                        </c:forEach>
                    </select>
                </div>
            </div>
                ...
                <div class="col-md-6">
                    <select path="idgerente" id="idgerente"
                        ...
                        <c:forEach items="${gerentes}" var="pessoa">
                            <option value="${pessoa.id}" ${projeto.idgerente == pessoa.id ? 'selected' :''} >${pessoa.nome}</option>
                        </c:forEach>
                    </select>
